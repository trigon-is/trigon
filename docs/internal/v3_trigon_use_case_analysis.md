...
This document grounds Trigon's abstract architecture in two concrete systems:
...
The goal is to answer: where does Trigon fit, what does it unlock, and what would
a production-grade multi-pipeline architecture look like — including a Kubernetes option.

---

## 2. How JSP V2 uses Trigon primitives today
...
The scout stage is already a Trigon-style invocation in everything but name:
...
### What Trigon would change for V2
...
```bash
# V2 pipeline with Trigon provider split (not yet implemented)

# Scout: cheap model, Playwright, structured JSON output
trigon-up.sh candidates/bergur/ \
...
stage reads it. No IPC. This is already how V2 works — Trigon just makes the
...
## 3. Trigon pipeline patterns applied to JSP
...
```bash
# init as a Trigon pipeline (conceptual)

# Stage 1: generate scout.md from CV + meta-prompt
trigon-up.sh candidates/anna/ \
  --provider anthropic \
...
The inter-stage context (`TRIGON_PLAN.md` equivalent) is the candidate folder
itself — each stage reads what the previous wrote. This already works today;
formalizing it as a Trigon pipeline adds the provider-switching option and
...
With Trigon, it's a one-liner at the shell level:

```bash
for candidate in bergur anna kristinn; do
  trigon-up.sh "candidates/${candidate}/" \
    --provider deepseek \
    --playwright \
...
with `claude-up.sh` — Trigon just gives it a clean API and the provider choice.

---

## 4. V3 SaaS: where Trigon sits in the architecture
...
[Trigon container] [Trigon container] [Trigon container]
     ↓                      ↓                       ↓
...
translates a queue job into a sequence of Trigon container runs. The AI
execution stays inside Trigon containers. The queue worker itself does not call
...
Trigon's unit model — one stateless container per pipeline stage — is naturally
...
| Trigon concept | Kubernetes equivalent |
|-------------------|-----------------------|
| `trigon-up.sh` invocation | `kubectl apply -f job.yaml` |
...
(MinIO, Hetzner Object Storage, AWS S3) can replace the PVC, with the Trigon
container reading/writing through a FUSE mount or an explicit sync step.

### 5.3 Kubernetes Job template

```yaml
# trigon-scout-job.yaml (generated by pipeline dispatcher)
apiVersion: batch/v1
kind: Job
metadata:
...
spec:
  ...
  template:
    spec:
      ...
      containers:
        - name: agent
          image: registry.example.com/trigon-claude-code:dev
          env:
...
All Trigon Jobs set `ANTHROPIC_BASE_URL=http://litellm.litellm.svc:4000`.
...
Burst pipeline runs spin up new nodes; idle periods scale down. The Trigon
...
Trigon's provider abstraction directly addresses this:
...
keys belong to the customer. Trigon's provider abstraction makes this trivial to
...
The `--air-gap` flag (air-gap mode) in Trigon has a specific V3 use case:
...
context. Trigon's mode system gives the answer:
...
The letter-writing Trigon container has `pdflatex` and `xelatex` available.
...
No separate LaTeX-compilation service needed. The Trigon container *is* the
LaTeX runner.

---

## 9. What Trigon does NOT solve for V3

Being clear about limitations:

- **Web UI** — Trigon is a headless pipeline tool. The browser UI, authentication,
  and user management in V3 are conventional web development; Trigon has no opinion.

- **Job board integrations** — The scout.md prompt drives board navigation. Trigon
...
- **Tenant data isolation at the database level** — Trigon provides filesystem-level
  isolation (per-candidate PVC). Postgres row-level security and tenant data model are
  standard web application concerns.

- **Real-time pipeline status** — Trigon containers emit stdout/stderr logs.
  Streaming these to a web UI in real time (WebSocket, SSE) requires work in the
  API server and pipeline dispatcher. Trigon itself is fire-and-forget.
...
Use Trigon's provider-switching (Option A from `trigon-feasibility.md`) to
route the scout stage to DeepSeek. No architectural change — just update
`pipeline-config.yaml` to call `trigon-up.sh` instead of `claude-up.sh` with
a `--provider deepseek` flag.

Expected effort: 1 day once `trigon-up.sh` exists. Immediate cost reduction.
...
for B2B career coaches by July 2026. At this stage, Trigon containers run on a
single VPS (Hetzner). Docker Compose is sufficient. No Kubernetes yet.

The pipeline dispatcher is a simple Celery worker that calls `subprocess.run(trigon-up.sh ...)`.
...
The Trigon container images are unchanged; only the launcher (pipeline dispatcher)
changes from shell subprocess to Kubernetes Job creation.

This migration is low-risk because Trigon's stateless-container-per-stage model
...
1. **Should `write_letters.py` become a Trigon container run?** Currently it calls
...
2. **How does the pipeline dispatcher know when a Trigon Job succeeds or fails?**
...
5. **Trigon as open-source and V3 as proprietary.** The strategic-overview raises
   open-source vs. moat. One viable model: Trigon (the harness) is open-source and
   builds community; JSP V3 (the product running on Trigon) is proprietary SaaS.
   This mirrors the open-core model and gives Trigon a real flagship use case to
   demonstrate against.
